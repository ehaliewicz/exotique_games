...
void enter_block(block* blk) {
    blk->cur_start_ticks = get_perf_counter();

}

void exit_block(block* blk) {
    blk->count++;
    blk->total_ticks = get_perf_counter() - blk->cur_start_ticks;
...
void print_and_reset_root_block(block *blk) {
    sys_printf("                 inc   inc per      exc   exc per\n", blk->name);
